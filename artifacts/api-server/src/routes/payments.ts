...
import { neonDb, cockroachDb, paymentsTable, salesTable, ordersTable, wholesaleSuppliesTable } from "@workspace/db";
...
router.get("/payments", async (req, res): Promise<void> => {
  ...
  if (qp.success && qp.data.date) {
    records = await neonDb.select().from(paymentsTable).where(sql`DATE(${paymentsTable.recordedAt}) = ${qp.data.date}`).orderBy(paymentsTable.recordedAt);
  } else {
    records = await neonDb.select().from(paymentsTable).orderBy(paymentsTable.recordedAt);
  }
...
router.post("/payments", async (req, res): Promise<void> => {
  ...
  const [payment] = await neonDb.insert(paymentsTable).values(parsed.data).returning();
...
router.patch("/payments/:id", async (req, res): Promise<void> => {
  ...
  const [existing] = await neonDb.select().from(paymentsTable).where(eq(paymentsTable.id, id));
  ...
  const [updated] = await neonDb.update(paymentsTable).set({
...
router.get("/payments/revenue-breakdown", async (req, res): Promise<void> => {
  ...
  const salesRows = await cockroachDb.select({
  ...
  const ordersRows = await neonDb.select({
  ...
  const wholesaleRows = await cockroachDb.select({
